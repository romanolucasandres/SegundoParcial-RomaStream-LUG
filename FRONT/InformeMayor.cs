...
namespace FRONT
{
    public partial class InformeMayor : Form
    {
        BLLLive live;
        BLLVOD vod;
        public InformeMayor()
        ...
        private void MostrarMayorStreamingEnChart()
        {
            try
            {
                live=new BLLLive();
                ...
                if (lista != null)
                {
                    var streamingPorCategoria = lista
                        ...
                        {
                            Categoria = group.Key,
                            MayorStreaming = group.OrderByDescending(s => s.Duracion).FirstOrDefault()
                        });

                    chart1.Titles.Clear();
                    chart1.Series.Clear();

                    chart1.Titles.Add("Mayor Streaming por Categoría");
                    ...
                    {
                        if (categoriaStreaming.MayorStreaming != null)
                        {
                            var streaming = categoriaStreaming.MayorStreaming;
        ...
        private void InformeMayor_Load(object sender, EventArgs e)
        {
            MostrarMayorStreamingEnChart();
        }
    }
}
